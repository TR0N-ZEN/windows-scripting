# winget install "Norton Security Protection"
# winget search 9NKK9HHB8HHF
# winget install 9NKK9HHB8HHF # "Norton Security Protection" from msstore
winget install "GnuPG.Gpg4win"
winget install "Git.Git"
winget install "Microsoft.VC++2015-2022Redist-x64"
winget install "7zip.7zip"

#winget install Mozilla.Firefox
winget install 9NZVDKPMR9RD #firefox from msstore
#winget install BraveSoftware.BraveBrowser
winget install "Mozilla.Thunderbird"
winget install "Mega.MEGASync"

#winget install Spotify.Spotify
winget install 9NCBCSZSJRSB # spotify from msstore

#winget install OBSProject.OBSStudio
#winget install OBSProject.obs-amd-encoder
# winget install "Discord.Discord"
winget install XPDC2RH70K22MN # discord from msstore

winget install "VideoLAN.VLC"

winget install "Microsoft.VisualStudioCode"
winget install "OpenJS.NodeJS.LTS"
winget install "Python.Python.3"
winget install "OpenVPNTechnologies.OpenVPN"
#winget install Microsoft.VisualStudio.2019.Community
#winget install BellSoft.LibericaJDK.17
#winget install JetBrains.IntelliJIDEA.Community
# winget install TimKosse.FileZilla.Client
# winget install Win32DiskImager
winget install "xournalplusplus"

#winget install Valve.Steam
#winget install Ubisoft.Connect
#winget install EpicGames.EpicGamesLauncher
#winget install ElectronicArts.EADesktop

winget install 9PMK584KQVC2 # Canon Inkjet Smart Connect